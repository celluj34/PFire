...
using PFire.Tests.Extensions;
using Xunit;

namespace PFire.Tests.PFire.Core.Protocol
{
    public class MessageAttributeSerializerTests : BaseTest
    {
        [Fact]
        ...
        public void WriteValue_WritesByte()
        {
            ...
            AssertWrittenValue(name, value, expected);
        }
        ...
        private void AssertReadValue<T>(byte[] bytes, T expected)
        {
            var messageSerializer = _autoMoqer.CreateInstance<MessageSerializer>();

            using var memoryStream = new MemoryStream(bytes);
            using var reader = new BinaryReader(memoryStream);
            var type = (XFireAttributeType)reader.ReadByte();
            var actual = messageSerializer.InvokePrivateMethod<dynamic>("ReadAttribute", reader, type);

            Assert.StrictEqual(expected, actual);
        }

        private void AssertWrittenValue<T>(string name, T value, byte[] expected)
        {
            var messageSerializer = _autoMoqer.CreateInstance<MessageSerializer>();
            ...
            using (var ms = new MemoryStream())
            {
                ...
                {
                    messageSerializer.InvokePrivateMethod("WriteAttribute", writer, new XMessageField(name), (object)value);
                }
